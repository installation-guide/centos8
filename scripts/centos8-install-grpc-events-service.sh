...
if [[ $is_overwrite == "Y" || $is_overwrite == "y" ]]; then
  ...
  export SERVICE_WORKING_FOLDER=${SERVICE_HOME};\
    envsubst< $SCRIPT_DIR/grpc/$SERVICE_NAME.sysconfig '${SERVICE_CPUS} ${SERVICE_CACHE_TYPE} ${SERVICE_PORT} ${SERVICE_USE_TLS} ${SERVICE_WORKING_FOLDER}'>  "$SYSCONFIG_PATH/$KAFKA_SERVICE.sysconfig"
