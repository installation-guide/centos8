...
SERVICE_NAME=${SERVICE_NAME:-"nav-structure-sync"}
...
if [[ $is_overwrite == "Y" || $is_overwrite == "y" ]]; then
  ...
  export SERVICE_WORKING_FOLDER=$SERVICE_HOME;
    cat $SCRIPT_DIR/nav/${SERVICE_NAME}.service | envsubst '${SERVICE_DESCRIPTION} ${SERVICE_USER} ${SERVICE_GROUP} ${SERVICE_SYSCONFIG} ${SERVICE_WORKING_FOLDER}' > "$SERVICE_SRC_SYSTEMD_PATH/$SERVICE_NAME.service"
